...
/**
 * Manages WebSocket connections for pushing real-time
 * notifications to connected frontend clients.
 *
 * Clients authenticate by passing `?userId=<uuid>` query param.
...
export class WsNotifier {
  ...
  constructor() {
    ...
    logger.info(`Notification WebSocket server listening on port ${config.wsPort}`);
  }
