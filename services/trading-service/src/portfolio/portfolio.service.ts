...
export class PortfolioService {
  ...
  // Injected at construction — direct call replaces inter-service HTTP
  ...
  async syncFromBroker(userId: string): Promise<{ synced: number }> {
    // Direct call — no HTTP hop since broker domain lives in the same process
    const connections = await this.brokerService.getConnections(userId);
    ...
    for (const conn of connections) {
      ...
      // Direct call — no HTTP hop
      const brokerHoldings = await this.brokerService.getHoldings(userId, conn.id) as any[];
