...
export class MarketDataService {
  ...
  async getQuote(exchange: Exchange, symbol: string): Promise<Quote> {
    ...
    try {
      ...
      console.error(`[MarketData] Trading-service quote fallback failed for ${exchange}:${symbol}:`, err.message);
    }

    throw new Error(`No data found for ${exchange}:${symbol}. Ensure a broker is connected and symbol master is synced.`);
  }
  ...
  // Expose raw candle fetch for in-process use by recommendations domain
  async getLatestCandles(symbol: string, exchange: Exchange, count: number): Promise<OHLCV[]> {
    return this.repo.getLatestCandles(symbol, exchange, count);
  }
}
