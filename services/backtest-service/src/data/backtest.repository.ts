...
export class BacktestRepository {
  ...
  async getModelMeta(modelId: string): Promise<ModelMetaRow | null> {
    const r = (await db('recommendations.model_registry').select('id', 'name').where({ id: modelId }).first()) as ModelMetaRow | undefined;
    return r ?? null;
  }
