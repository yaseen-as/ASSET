...
export class PerformanceBackfillCron {
  ...
  async runOnce(): Promise<{ updated: number }> {
    // Single SQL pass per horizon. Joins to ohlcv_daily for entry close and
    // the close H trading days ahead (approximated via calendar offset with a
    // sub-select picking the nearest available bar; conservative if a symbol
    // skipped a day due to a halt).
