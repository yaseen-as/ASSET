"""Evaluate the trained technical model on the held-out test split.

Reports:
  - AUC
  - Precision@K (top-K ranked predictions per day)
  - Lift (precision@K / baseline positive rate)
  - Per-day decile breakdown
"""
...
def precision_at_k_per_day(df: pd.DataFrame, k: int) -> float:
    """Mean precision@K across trading days."""
...
def main() -> None:
    ...
    parser.add_argument("--training-set", default="artifacts/training_set.parquet")
    parser.add_argument("--model", default="artifacts/technical_model.txt")
    parser.add_argument("--meta", default="artifacts/technical_model.meta.json")
    parser.add_argument("--out", default="artifacts/technical_model.eval.json")
    ...
    deciles = (
        test.groupby("decile")["label"].mean().to_dict()
        if "decile" in test.columns else {}
    )
